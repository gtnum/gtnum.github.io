...
var flowSVG = (function () {
          ...
          // Set defaults
            var w = userOpts.w || 180,
                ...
                defaults = {
                    ...
                    labelYes: userOpts.labelYes ||  'Oui',
                    labelNo: userOpts.labelNo ||  'Non',
                    labelJnsp: userOpts.labelJnsp ||  'Jnsp',
        ...
        function yesNoIds(element) {
            ...
            if (element.jnsp) {
                element.jnspid = itemIds[element.jnsp];
                element.svgjnspid = SVG.get(itemIds[element.jnsp]);
            }
            if (element.next) {
        ...
        function referringIds(element) {
            ...
            if (!element.orient) {
                element.orient = {yes: 'b', no: 'r', jnsp: 'l', next: 'b'};
            }
        ...
        function staticNodePoints(element) {
            ...
            if (element.jnsp && element.jnspid !== undefined) {
                targetShape = shapes[lookup[element.jnsp]];
                te = element.svgjnspid;

                if (element.orient.jnsp === 'b') {
                    element.jnspOutPos = [ce.cx(), ce.cy() + ce.get(0).cy()];
                    targetShape.inNode = targetShape.inNode !== undefined ? targetShape.inNode : 't';
                    targetShape.inNodePos = [te.cx(), te.y()];
                }
                ...
                if (element.orient.jnsp === 'r') {
                    element.jnspOutPos = [ce.cx() + ce.get(0).cx(), ce.cy()];
                    targetShape.inNode = targetShape.inNode !== undefined ? targetShape.inNode : 'l';
                ...
                isPositioned.push(element.jnspid);
            }
     ...
     if (element.jnsp && element.jnspid !== undefined) {
                targetShape = shapes[lookup[element.jnsp]];
                te = shapes[lookup[element.jnsp]].svgshape;

                if (element.orient.jnsp === 'b') {
                    element.jnspOutPos = [ce.cx(), ce.y() + ce.bbox().height];
                    targetShape.inNode = targetShape.inNode !== undefined ? targetShape.inNode : 't';
                    targetShape.inNodePos = [te.cx(), te.y()];
                }

                if (element.orient.jnsp === 'l') {
                    element.jnspOutPos = [ce.x() + ce.bbox().width, ce.cy()];
                    targetShape.inNode = targetShape.inNode !== undefined ? targetShape.inNode : 'l';
      ...
      if (element.jnsp && element.jnspid !== undefined) {
                label = lineLabel(config.labelJnsp, group);
                element.jnspBtn = label;
                label.attr('cursor', 'pointer');
                label.on('click', function () {
                    toggleNext(element, 'jnsp');
                });

                if (element.orient.jnsp === 'b') {
                    label.move(element.jnspOutPos[0] + config.labelNudgeBottom, element.jnspOutPos[1]);
                }

                 if (element.orient.jnsp === 'l') {
                     label.move(element.jnspOutPos[0] - 100 + config.labelNudgeBottom, element.jnspOutPos[1]);
                 }


                if (element.orient.jnsp === 'r') {
                    label.move(element.jnspOutPos[0] + 20 + config.labelNudgeRight, element.jnspOutPos[1] - 20);
                }
            }
        }
	    ...
	    if (element.jnsp && element.jnspid !== undefined) {
                ...
                if (element.orient.jnsp === 'l') {
                    label.move(element.jnspOutPos[0] -20 + config.labelNudgeRigth, element.jnspOutPos[1]);
                }

		if (element.orient.jnsp === 'b') {
                     label.move(element.jnspOutPos[0], element.jnspOutPos[1]);
                 }
        ...
        function addArrows(element) {
            var group = element.svgid, arrowhead, rightX, rightY, bottomX, bottomY, leftX, leftY;
	    ...
	    leftX = element.svgshape.x() - element.svgshape.bbox().width + config.connectorLength - config.arrowHeadHeight;
	    leftY = element.svgshape.cy() - (config.arrowHeadHeight / 2);	
            ...
            if (element.svgyesid !== undefined) {
                if (element.orient.yes === 'r') {
                    ...
                    arrowhead.move(svgshape.cy() - (config.arrowHeadHeight / 2));
         ...
         if (element.svgjnspid !== undefined) {
                if (element.orient.jnsp === 'r') {
                    arrowhead = arrowHead(group);
                    arrowhead.move(rightX, rightY);
                    arrowhead.rotate(270);
        ...
        function staticAddArrows(element) {
            ...
            if (element.no && element.noid !== undefined) {
                ...
                if (element.orient.no === 'r') {
                    arrrowhead.move(nxt.inNodePos[0] - config.arrowHeadHeight, nxt.inNodePos[1] - (config.arrowHeadHeight / 2));
                    if(nxt.inNode === 'l') {
                        arrowhead.rotate(270);
                    }
          ...
          if (element.jnsp && element.jnspid !== undefined) {
                arrowhead = arrowHead(group);
                nxt = shapes[lookup[element.jnsp]];
      ...
      if (element.jnspid) {
       ...
       if (element.jnspid) {
                startln = element.jnspOutPos;
                endln = shapes[lookup[element.jnsp]].inNodePos;
                element.conngroup.polyline(angleLine(startln, endln, element, element.jnspid)).stroke({ width:  config.connectorStrokeWidth, color: config.connectorStrokeColour}).fill('none');
            }
